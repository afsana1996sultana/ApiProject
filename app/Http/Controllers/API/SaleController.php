...
use App\Http\Resources\SaleResourceIndex;
...
class SaleController extends Controller
{
    ...
    public function index()
    {
        ...
        return SaleResourceIndex::collection($filteredPaginatedSales)
            ->additional([
    ...
    {
        $sales = Sale::with(['getCourier', 'getCustomer'])->latest()->paginate(10);
        $saleList = [];
    ...
    public function store(SaleRequest $request)
    {
        ...
        try {
            ...
            //sale Invoice : Serial - 0000000015
            $lastSaleInvoice = Sale::orderBy('id', 'desc')->select('invoice')->first();
            if ($lastSaleInvoice) {
                preg_match('/(\d+)$/', $lastSaleInvoice->invoice, $matches);
                $lastInvoiceNumber = isset($matches[1]) ? (int)$matches[1] : 0;
            } else {
                $lastInvoiceNumber = 0;
            }
            $createInvoiceNumber = str_pad($lastInvoiceNumber + 1, 12, '0', STR_PAD_LEFT);

            // sale Invoice : Timestamp - 09042024121
            // $createInvoiceNumber = Str::random(6) . Carbon::now()->format('His');
            
            //  function generateUniqueInvoiceNumber() {
            //     do {
            //         $createInvoiceNumber = Str::random(15);
            //     } while (Sale::where('invoice', $createInvoiceNumber)->exists());

            //     return $createInvoiceNumber;
            // }

            $invoice = $sale->invoice = "IS-" . $createInvoiceNumber;
            $barcodePath = public_path("barcodes/{$invoice}.png");
